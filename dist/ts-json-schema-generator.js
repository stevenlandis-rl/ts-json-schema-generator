...
try {
    ...
    const schemaString = config.sortProps ? (0, json_stable_stringify_1.default)(schema, { space: 2 }) : JSON.stringify(schema, null, 2);
    if (args.out) {
        (0, fs_1.writeFile)(args.out, schemaString, (err) => {
            if (err)
                throw err;
        });
    }
